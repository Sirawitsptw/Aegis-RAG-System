...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # อนุญาตให้ทุกเว็บยิงเข้ามาได้ (ใช้ตอน Dev)
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# 1. สร้างความจำ (ทำแค่รอบเดียวตอนรันเซิร์ฟเวอร์)
...
async def analyze_issue(issue: CustomerIssue):
    ...
    # 2.4 ส่งผลลัพธ์กลับไปให้หน้าบ้าน
    return {
        "status": "success",
        "raw_message": issue.message,
        "matched_manual": retrieved_context, # ส่งคู่มือที่อ้างอิงกลับไปให้ Dev ดูด้วย
